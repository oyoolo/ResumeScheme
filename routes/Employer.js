...
const { ensureLoggedOut, ensureLoggedIn } = pkg;
...
//Register
router.get("/register", ensureLoggedOut("/dashboard"), (req, res) =>
  res.render("employersregister")
);
router.post("/register", new EmployerController().register);

//New Job
router.post("/post_job", ensureLoggedIn(), new EmployerController().postJob);
...
router.get(
  "/:jobID/applicants",
  ensureLoggedIn(),
  new EmployerController().getApplicants
);
