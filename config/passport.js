...
export default function (passport) {
  ...
  passport.use('local-jobseeker', new LocalStrategy({
    usernameField: 'email'
  },
    async (email, password, done) => {
  ...
  passport.deserializeUser(
    async (sessionConstructor, done) => {
      if (sessionConstructor.userType === 'employer') {
        await Employer.findById(sessionConstructor.userId, '-localStrategy.password',
          (err, user) => {
            done(err, user);
            if (err)
              throw err;
        ...
        await JobSeeker.findById(sessionConstructor.userId, '-localStrategy.password',
          (err, user) => {
            done(err, user);
            if (err)
              throw err;
